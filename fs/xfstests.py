...
class Xfstests(Test):
    ...
    def test(self):
        ...
        if self._test_number == '000':
            self.log.info('Dummy test to setup xfstests')
            return

        if self._test_number not in self._get_available_tests():
            raise self.error('test file %s not found' % self._test_number)

        self.log.info("Running test: %s" % self._test_number)
