...
class ParallelDd(Test):
    ...
    def setUp(self):
        ...
        root_fs_device = process.system_output(root_fs_device_cmd, shell=True)
    ...
    def _device_to_fstype(self, s_file, device=None):
        ...
        try:
            ...
            logging.debug(line)
            fstype = line.split()[2]
            logging.debug('Found %s is type %s from %s', device, fstype,
                          s_file)
            return fstype
        except process.CmdError:
            logging.error('No %s found in %s', device, s_file)
            return None
