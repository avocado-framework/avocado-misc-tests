...
class NVMeTest(Test):
    ...
    def setUp(self):
        ...
        # cmd = "ls -l %sn* | sort | awk '{print $NF}'" % self.device
        # self.id_ns = process.system_output(cmd, shell=True).strip('\n'):
        # if 'No such file or directory' in self.id_ns:
        #     self.id_ns = self.device + 'n1'
        ...
        print self.device, self.id_ns, self.lba, self.format_size
    ...
    def delete_namespace(self):
        ...
        """
        cmd = "nvme show-regs /dev/nvme0 | grep version | awk '{print $NF}'"
        if int(process.system_output(cmd, shell=True).strip('\n')) < 10200:
            return
    ...
    def testread(self):
        ...
        """
        cmd = 'nvme read %s -z %d' % (self.id_ns, self.format_size)
        process.run(cmd, shell=True)
    ...
    def testwrite(self):
        ...
        """
        cmd = 'nvme write %s -z %d' % (self.id_ns, self.format_size)
        process.run(cmd, shell=True)
