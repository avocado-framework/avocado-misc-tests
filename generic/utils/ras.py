...
class RASTools(Test):
    ...
    def test(self):
        ...
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        command_to_run = (os.path.join(self.datadir, script_to_run) + ' ' +
                          '>>' + ' ' + os.path.join(self.datadir, "log", "output.txt") + ' ' + '2>&1')
        cmd_result = process.run(command_to_run, ignore_status=True, sudo=True)
