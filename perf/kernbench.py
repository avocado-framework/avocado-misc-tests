...
#
# Copyright: 2016 IBM
# Author: Harish <sudeesh@linux.vnet.ibm.com>
#
...
class KernbenchTest(Test):
    ...
    """
    def time_build(self, kern, threads=None, timefile=None, make_opts=None, output=None):
    ...
    def setUp(self, version=None):
        ...
        for package in deps:
            if sm.check_installed(package) and not sm.install(package):
    ...
    def extract_all_time_results(self, results_string, check=None):
    ...
    def test(self):
        ...
        Kernel build Test
        """
        config = self.params.get('config_file', default='config')
        config_path = os.path.join(self.datadir, config)
        ...
        if config is None:
            # Setting default config file as the 'config' in data Directory
            config_path = os.path.join(self.datadir, config)
        # initialize kernel object
        kern = kernel.KernelBuild(version, config_path=config_path, work_dir=None, data_dirs=None)
        ...
        outfile = ("%s/out_file" % kern.build_dir)
        # Build kernel
        user_time = 0
        system_time = 0
        elapsed_time = 0
        for runs in range(iterations):
            self.time_build(kern, threads, timefile,"", outfile)
            # Processing the timefile
            results = open(timefile).readline()
