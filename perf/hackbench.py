...
class Hackbench(Test):
    ...
    def test(self):
        ...
        time_value = self.params.get('time_val', default=None)
        num_groups = self.params.get('num_groups', default=10)
        ...
        for line in self.results.split('\n'):
            if line.startswith('Time:'):
                ...
                time = time_val
                perf_json = {'time': time_val}
        output_path = os.path.join(self.outputdir, "perf.json")
        json.dump(perf_json, open(output_path, "w"))
        self.log.info("Time Taken:" + time)
        if time_value:
            if time_value != time:
                self.error("Test failed: Time Taken not "
                           "matched with threshold")
