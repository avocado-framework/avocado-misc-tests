...
class Sensors(Test):
    ...
    def test(self):
        ...
        process.run('sensors-detect --auto', sudo=True)
        error_list = self.check_errors('sensors')
        if len(error_list) > 0:
            self.error('sensors command failed with %s' % error_list)
        error_list = self.check_errors('sensors -f')
        if len(error_list) > 0:
            self.error('sensors -f command failed with %s' % error_list)
        error_list = self.check_errors('sensors -A')
        if len(error_list) > 0:
            self.error('sensors -A command failed with %s' % error_list)
        error_list = self.check_errors('sensors -u')
        if len(error_list) > 0:
            self.error('sensors -u command failed with %s' % error_list)
